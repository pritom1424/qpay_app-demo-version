...
class MobileRechargeOtpPage extends StatefulWidget {
  final TransactionViewModel _transaction;
  final TransactionAmountDescriptionWidget header;
  MobileRechargeOtpPage(this._transaction, this.header);

  @override
  _MobileRechargeOtpPageState createState() =>
      _MobileRechargeOtpPageState(_transaction, header);
}

class _MobileRechargeOtpPageState extends State<MobileRechargeOtpPage>
    with
        BasePageMixin<MobileRechargeOtpPage, MobileRechargePresenter>,
        AutomaticKeepAliveClientMixin<MobileRechargeOtpPage>
    implements MobileRechargeIMvpView {
  TransactionViewModel? _transaction;
  TransactionAmountDescriptionWidget header;
  _MobileRechargeOtpPageState(this._transaction, this.header);
  ...
  void _onPinInserted(String pin) async {
    ...
    var response = await presenter.confirmTransaction(
        _transaction?.transactionId ?? '', pin);
