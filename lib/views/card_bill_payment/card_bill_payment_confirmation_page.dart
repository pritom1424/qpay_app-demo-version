...
class CardBillPaymentConfirmationPage extends StatefulWidget{
  final double amount;
  ...
  CardBillPaymentConfirmationPage(this.amount, this.feeAmount,this.vatAmount, this.from, this.to,
      this.selectedDebitAccount, this.selectedCreditAccount, this.purpose,this.cvv, this.isOtpRequired);

  @override
  _CardBillPaymentConfirmationPageState createState() => _CardBillPaymentConfirmationPageState(selectedDebitAccount,selectedCreditAccount,amount,feeAmount,vatAmount,from,to,purpose,cvv,isOtpRequired);
}

class _CardBillPaymentConfirmationPageState extends State<CardBillPaymentConfirmationPage>
    with
        BasePageMixin<CardBillPaymentConfirmationPage, CardBillPaymentPresenter>,
        AutomaticKeepAliveClientMixin<CardBillPaymentConfirmationPage>
  ...
  _CardBillPaymentConfirmationPageState(
  ...
  void _initiate() async {
    ...
    final int? cardNumber = _selectedCreditAccount.id;
    final String purpose = _purpose;
    final String cvv = _cvv;
    var response = await presenter.creditCardBillPayment(
        amount, debitAccountId!, cardNumber!, accountType,purpose,_cvv);
        ...
        if (_transaction!.isRedirectAllow!) {
          var result = await Navigator.push(
            ...
            return WebViewGatewayPage(AppLocalizations
                .of(context)
                !.cardBillPayment, _transaction!.redirectUrl!);
          }));
  ...
  Future<bool?> _showPinDialog(TransactionViewModel? transaction) async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (context) {
      return CardBillPaymentOtpPage(transaction,TransactionAmountDescriptionWidget(transaction));}));
  ...
  @override
  CardBillPaymentPresenter createPresenter() => CardBillPaymentPresenter();
